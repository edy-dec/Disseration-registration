...
import { professorOnly, studentOnly, requireCompleteProfile } from '../middleware/roleCheck.js';

const router = express.Router();

// GET /api/users/students - Lista studenti (doar pentru profesori)
router.get('/students', authenticate, professorOnly, requireCompleteProfile, async (req, res) => {
  try {
    ...
    ).sort({ 'studentDetails.faculty': 1, 'studentDetails.year': 1, name: 1 });
...
// GET /api/users/professors - Listă profesori (pentru toți utilizatorii autentificați)
router.get('/professors', authenticate, requireCompleteProfile, async (req, res) => {
  try {
...
router.get('/stats', authenticate, professorOnly, requireCompleteProfile, async (req, res) => {
  try {
    ...
    const studentsCount = stats.find(s => s._id === 'student') || { total: 0, profileComplete: 0 };
    const professorsCount = stats.find(s => s._id === 'profesor') || { total: 0, profileComplete: 0 };
    ...
    res.json({
      ...
      stats: {
        students: {
          ...
          profileIncomplete: studentsCount.total - studentsCount.profileComplete
        },
        professors: {
          total: professorsCount.total,
          profileComplete: professorsCount.profileComplete,
          profileIncomplete: professorsCount.total - professorsCount.profileComplete
        }
      }
    });
