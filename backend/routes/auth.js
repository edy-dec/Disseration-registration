...
router.post('/login', validateLogin, checkValidation, async (req, res) => {
  try {
    ...
    if (user.provider !== 'local') {
      return res.status(400).json({
        success: false,
        message: 'Acest cont foloseste autentificare externa. Foloseste butonul de login corespunzator.'
      });
    }
...
router.put('/complete-profile', authenticate, async (req, res) => {
  try {
    ...
    if (user.userType === 'student') {
      ...
      await Promise.all(validateStudentProfile.map(validation => validation.run(req)));
      ...
      await Promise.all(validateProfessorProfile.map(validation => validation.run(req)));
      const errors = checkValidation(req, res, () => {});
      if (res.headersSent) return; 
